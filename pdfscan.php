...
function split_into_files_of_each_user($currentfolder,$origfile){
  ...
  for($co=1; $co<10000; $co++){
	...
	if (!is_null($lastQRfile)){
	  ...
	  exec("mkdir $burstfolder"."/cat/");
	  copy($filenamefull, $burstfolder."/cat/".$filename);
	  unlink($filenamefull);
	}
      }
...
function decode_qr($pdffile){
  global $gs_command, $convert_command, $zbarimg_command, $tmpd, $mailmsg;
  preg_match('|/(\w+)[.]pdf$|',$pdffile,$match);  $fnamebase= $match[1];
  ...
  $execcmd1="$gs_command -dSAFER -dBATCH -dNOPAUSE -sDEVICE=jpeg -r400 -sOutputFile=$tmpjpg1 -dFirstPage=1 -dLastPage=1 $pdffile";
  exec($execcmd1);
...
//  $execcmd2="$convert_command -blur 1 $tmpjpg1 $tmpjpg2";
  $execcmd2="$convert_command -white-threshold 98% -geometry 2400x3600 -crop 2000x1000+60+60 $tmpjpg1 $tmpjpg2";
  exec($execcmd2);
  $execdmtx = "$zbarimg_command -q --raw $tmpjpg2";
  $outs = $userid = $asno = $coursename = $username = NULL;
  exec($execdmtx,$outs,$ret);   $outstr= implode("",$outs);
  ...
  $mailmsg .= "\npdfscan.php, " . $execcmd1 .", " .$execcmd2 .", " . $execdmtx . ", (" . $outstr . "), // userid:" . $userid . ",asno:" . $asno . ",". $coursename . "," . $username . ",";
...
function submit_file($filename,$currentfolder){
  ...
  */
  ...
  // Get file
  $cfile = $fs->get_file($fileinfo['contextid'], $fileinfo['component'], $fileinfo['filearea'], $fileinfo['itemid'], $fileinfo['filepath'], $fileinfo['filename']);
  ...
  if ( ! $DB->get_record('assignsubmission_file', array('assignment'=>$filesubmission->assignment, 'submission'=>$filesubmission->submission)) ){
